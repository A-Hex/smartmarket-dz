...
Backend test suite: **118/118 passing** (`cd backend && pytest -q`), plus 2 real-Postgres integration tests
(skipped by default, see "A real production bug and what it means for this README" below). Frontend: TypeScript
strict + ESLint clean, 6/6 Vitest passing, full `next build` succeeds (36 statically generated routes across
fr/ar/en). All 10 acceptance criteria from section 18 verified end-to-end — see below.

## A real production bug, and what it means for this README

**Update, post-deployment:** a bug shipped that every test in this project failed to catch, because it only
manifests against real PostgreSQL, and every test here (unit, API, and the acceptance-criteria e2e runs)
executed against an in-memory **SQLite** database via the portable `GUID`/`JSONBType` column types built for
test-speed and no-Docker-needed convenience.

**The bug:** SQLAlchemy's `Enum` column type defaults to persisting a Python enum member's `.name` (e.g.
`"OWNER"`) rather than its `.value` (`"owner"`). Every model here defines enums as `class Foo(str,
enum.Enum): BAR = "bar"` (uppercase names, lowercase values), and every one of the 11 enum columns across the
schema used plain `Enum(PythonEnumClass, name="...")` with no `values_callable` override. SQLite doesn't
enforce a real ENUM constraint, so both writing and reading used `.name` consistently and nothing ever looked
wrong. Real Postgres, via a native ENUM type, strictly rejects any value outside the exact set defined in the
migration (lowercase, matching each enum's `.value`) — so the very first `INSERT` with an enum column
(registering a user, `role='OWNER'`) failed with `InvalidTextRepresentationError: invalid input value for enum
user_role: "OWNER"`, surfacing as a bare 500 to the frontend. This was reproduced against a real local Postgres
instance and fixed by adding `values_callable=lambda x: [e.value for e in x]` to all 11 `Enum(...)` column
definitions in `app/models/*.py`.

**Why this matters beyond the fix itself:** "all tests pass," even across a scripted multi-step acceptance
walkthrough, is only as trustworthy as the infrastructure it runs against. This project's tests were fast and
thorough but had a blind spot baked into their foundation, and it took a real deployment to surface it.
`tests/test_postgres_integration.py` now exists specifically to close this gap — skipped by default (no
Postgres dependency for routine `pytest` runs) but should be run against a real Postgres instance before any
deploy:
```bash
createdb smartmarket_test
export TEST_POSTGRES_URL=postgresql+asyncpg://USER:PASS@localhost:5432/smartmarket_test
cd backend && alembic upgrade head && pytest tests/test_postgres_integration.py -v
```
Treat every "✅ Done" and "verified end-to-end" claim elsewhere in this README as verified **against SQLite**
unless stated otherwise — accurate for logic correctness, but not a substitute for testing against the actual
